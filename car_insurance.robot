*** Settings ***
Library    Browser
Library    DateTime    # Added datetime for insurance startdate
...
Enter Vehicle Data for Automobile
    ...
    Wait For Elements State    section[style*="display: block"]    visible    10s     # Added this so that the section has time to become visible
...
Enter Product Data
    ${future_date}=    Get Current Date    result_format=%m/%d/%Y    increment=30 days    # the startdate needs to be at least 30 days in the future
    Fill Text    id=startdate    ${future_date}
...
Send Quote
    ...
    ${promise}=     Promise To    Wait For Response     matcher=https://sampleapp.tricentis.com/101/tcpdf/pdfs/quote.php     timeout=10    # changed to https
    Click    "« Send »"
